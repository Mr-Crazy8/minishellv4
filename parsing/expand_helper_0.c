...
/*   Updated: 2025/05/19 11:26:51 by anel-men         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    if (current->args)
        {
            ...
            while (current->args[(*i)])
            {
                if (current->args[(*i)] && current->args_befor_quotes_remover[(*i)][0] == '$' && (strchr(current->args[(*i)], '\'') != NULL || strchr(current->args[(*i)], '\"') != NULL))
                    remove_mode = 0;
                processed = selective_remove_quotes(current->args[(*i)], remove_mode);
                if (processed)
                {
